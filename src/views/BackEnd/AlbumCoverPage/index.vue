  ...
  <div class="alubumCoverPage">
    ...
    <div class="table_zone">
      <div class="table_header sticky-top d-flex align-items-center border-bottom border-top">
        ...
        <div data-column='coverName'>
          <span class="font_14">封面名稱</span>
          <div class="font_12 d-flex align-items-center subTitle">
            <span class="font_12 mr-2">Cover Name</span>
          </div>
        </div>
      ...
      <table class="table mb-3 position-relative">
        <tbody>
          <tr v-for="item in datatables" :key="item.id">
            ...
            <td class="font_12" data-column='coverName'>{{item.coverName}}</td>
            <td class="font_12" data-column='descript'>{{item.description}}</td>
            <td class="font_12" data-column='status'>
              <div class="d-flex align-items-center coverStatus" :class="{'inactive' : item.status === 0}">
...
export default {
  ...
  computed: {
    // vuex
    ...mapGetters('albumCoverTableStore', ['datatables', 'perPage', 'count', 'totalPages', 'totalCount', 'currentPage', 'paramsObj', 'paramsStatus', 'trashStatus'])
  },
  filters: {
    statusFilter (val) {
  ...
  methods: {
    // vuex
    ...mapActions('albumCoverTableStore', ['initData', 'getDatatable', 'getRequestParams', 'setParamsStatus', 'setTrashStatus']),
    async init () {
      this.initData()
